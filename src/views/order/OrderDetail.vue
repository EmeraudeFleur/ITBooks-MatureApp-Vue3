<template>
  <div class="order-detail-box">
    <nav-bar class="nav-bar">
      <template v-slot:default> Order Detail </template>
    </nav-bar>
    ...
    >
      <div :style="{ width: '90%', margin: '0 auto', padding: '50px 0' }">
        <van-grid :border="false" :column-num="2">
          <van-grid-item text="AliPay">
            AliPay QRcode<br />
            <img width="150" height="150" :src="aliyun" />
          </van-grid-item>
          <van-grid-item text="Wechat">
            Wechat QRcode<br />
            <img width="150" height="150" :src="wechat" />
          </van-grid-item>
        </van-grid>
      </div>
    </van-popup>
...
export default {
  ...
  setup() {
    ...
    const statusString = computed(() => {
      // 1下单 2支付 3发货 4收货 5过期
      let status = ["", "ordered", "paid", "dispatched", "received", "expired"];
